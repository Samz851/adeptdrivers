...
// Current version
if ( ! defined( 'TRX_ADDONS_VERSION' ) ) define( 'TRX_ADDONS_VERSION', '1.6.18.1' );
...
if ( !function_exists( 'fsdriving_specific_sc_form_ajax_send_sc_form' ) ) {
    add_action('wp_ajax_send_sc_form',			'fsdriving_specific_sc_form_ajax_send_sc_form');
    add_action('wp_ajax_nopriv_send_sc_form',	'fsdriving_specific_sc_form_ajax_send_sc_form');
    function fsdriving_specific_sc_form_ajax_send_sc_form() {
        ...
        if (!($contact_email = get_option('admin_email')))
            $response['error'] = esc_html__('Unknown admin email!', 'fsdriving');
        else {
            ...
            $user_subject	= !empty($post_data['subject']) ? stripslashes($post_data['subject']) : sprintf(__('Site %s - Contact form message from %s', 'fsdriving'), get_bloginfo('site_name'), $user_name);
            ...
            $subj = $user_subject;
            $msg = (!empty($user_name)	? "\n".__('Name:', 'fsdriving')   .' '.trim($user_name) : '')
                .  (!empty($user_email) ? "\n".__('E-mail:', 'fsdriving') .' '.trim($user_email) : '')
                .  (!empty($user_phone) ? "\n".__('Phone:', 'fsdriving')  .' '.trim($user_phone) : '')
                .  (!empty($user_msg)	? "\n".__('Message:', 'fsdriving')."\n".trim($user_msg) : '')
                .  "\n\n............. " . get_bloginfo('site_name') . " (" . esc_url(home_url('/')) . ") ............";

            if (!wp_mail($contact_email, $subj, $msg)) {
                $response['error'] = esc_html__('Error send message!', 'fsdriving');
            }
