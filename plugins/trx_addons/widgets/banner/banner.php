...
class trx_addons_widget_banner extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		set_query_var('trx_addons_args_widget_banner', array_merge($args, compact('title', 'fullwidth', 'banner_image', 'banner_link', 'banner_code')));
		if (($fdir = trx_addons_get_file_dir('widgets/banner/tpl.default.php')) != '') { include $fdir; }
	}
...
if ( !function_exists( 'trx_addons_sc_widget_banner' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_banner", "trx_addons_sc_widget_banner");
}
...
if (!function_exists('trx_addons_sc_widget_banner_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('init', 'trx_addons_sc_widget_banner_add_in_vc', 20);
}
?>