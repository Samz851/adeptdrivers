...
class trx_addons_widget_video extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		set_query_var('trx_addons_args_widget_video', array_merge($args, compact('title', 'embed', 'link', 'cover')));
		if (($fdir = trx_addons_get_file_dir('widgets/video/tpl.default.php')) != '') { include $fdir; }
	}
...
if ( !function_exists( 'trx_addons_sc_widget_video' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_video", "trx_addons_sc_widget_video");
}
...
if (!function_exists('trx_addons_sc_widget_video_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('init', 'trx_addons_sc_widget_video_add_in_vc', 20);
}
?>