...
class trx_addons_widget_popular_posts extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ( $tabs[0]['title'].$tabs[0]['content'].$tabs[1]['title'].$tabs[1]['content'].$tabs[2]['title'].$tabs[2]['content'] ) {
			...
			set_query_var('trx_addons_args_widget_popular_posts', array_merge($args, compact('title', 'tabs', 'tabs_count')));
			if (($fdir = trx_addons_get_file_dir('widgets/popular_posts/tpl.default.php')) != '') { include $fdir; }
...
if ( !function_exists( 'trx_addons_sc_widget_popular_posts' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_popular_posts", "trx_addons_sc_widget_popular_posts");
}
...
if (!function_exists('trx_addons_sc_widget_popular_posts_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('init', 'trx_addons_sc_widget_popular_posts_add_in_vc', 20);
}
?>