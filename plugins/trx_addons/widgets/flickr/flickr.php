...
class trx_addons_widget_flickr extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		set_query_var('trx_addons_args_widget_flickr', array_merge($args, compact('title', 'flickr_username', 'flickr_count')));
		if (($fdir = trx_addons_get_file_dir('widgets/flickr/tpl.default.php')) != '') { include $fdir; }
	}
...
if ( !function_exists( 'trx_addons_sc_widget_flickr' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_flickr", "trx_addons_sc_widget_flickr");
}
...
if (!function_exists('trx_addons_widget_flickr_reg_shortcodes_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('init', 'trx_addons_widget_flickr_reg_shortcodes_vc', 20);
}
?>