...
if (!function_exists('trx_addons_cpt_services_init')) {
	...
	function trx_addons_cpt_services_init() {
		...
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_SERVICES_PT] = array(
			...
			"image_icon" => array(
				"title" => esc_html__("Item's image icon", 'trx_addons'),
				"desc" => wp_kses_data( __('Select image icon for the current service item', 'trx_addons') ),
				"std" => "",
				"options" => trx_addons_array_merge(array('none' => esc_html__("No image icon", 'trx_addons')), trx_addons_get_list_icons()),
				"type" => "image"
			)
		);
...
if (!function_exists('trx_addons_sc_services_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('init', 'trx_addons_sc_services_add_in_vc', 20);
}
?>