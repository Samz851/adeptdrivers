...
if ( !function_exists('fsdriving_need_page_title') ) {
	function fsdriving_need_page_title() {
		return !is_front_page() && apply_filters('fsdriving_filter_need_page_title', true);
	}
}
...
if ( !function_exists('fsdriving_show_layout') ) {
	function fsdriving_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('fsdriving_get_logo_image') ) {
	function fsdriving_get_logo_image($type='') {
		$logo_image = '';
		if (fsdriving_get_retina_multiplier(2) > 1)
			$logo_image = fsdriving_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = fsdriving_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('fsdriving_get_header_video') ) {
	function fsdriving_get_header_video() {
		$video = '';
		if (apply_filters('fsdriving_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (fsdriving_check_theme_option('header_video')) {
				$video = fsdriving_get_theme_option('header_video');
...
if ( !function_exists('fsdriving_show_post_meta') ) {
	function fsdriving_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('fsdriving_filter_get_post_date', fsdriving_get_date());
				if (!empty($dt)) {
					?>
					<span class="post_meta_item post_date<?php if (!empty($args['seo'])) echo ' date updated'; ?>"<?php if (!empty($args['seo'])) echo ' itemprop="datePublished"'; ?>><?php echo esc_html__('Posted on: ', 'fsdriving'); ?><a href="<?php echo esc_url(get_permalink()); ?>"><?php echo esc_html($dt); ?></a></span>
					<?php
				}
			}
			// Post categories
			if ( !empty($args['categories'])) {
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('fsdriving_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php fsdriving_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post author
			...
			// Post counters
			fsdriving_show_layout(fsdriving_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				fsdriving_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'fsdriving'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'fsdriving' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('fsdriving_show_post_featured') ) {
	function fsdriving_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => fsdriving_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => fsdriving_is_on(fsdriving_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : fsdriving_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = fsdriving_get_post_image();
				if (!empty($image)) 
					$image = fsdriving_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = fsdriving_get_no_image();
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . fsdriving_add_inline_style('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$fsdriving_attr = fsdriving_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($fsdriving_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($fsdriving_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						fsdriving_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = fsdriving_build_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						fsdriving_show_layout($output);
					...
					if (!empty($image)) {
						...
						fsdriving_hovers_add_icons($args['hover'], array('image' => $image));
					}
				} else if (!empty($args['show_no_image']) && ($image=fsdriving_get_no_image())!='') {
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = fsdriving_get_post_video($post_content, false);
						if (empty($video))
							$video = fsdriving_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = fsdriving_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = fsdriving_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php
							}
							?><div class="post_video video_frame"><?php
								if ( !$has_thumb ) {
									fsdriving_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = fsdriving_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = fsdriving_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = fsdriving_array_get_first($audios, false);
								}
							}
						}
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php
								...
								$media_author = fsdriving_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = fsdriving_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !fsdriving_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php fsdriving_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !fsdriving_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php fsdriving_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								fsdriving_show_layout($audio);
							?></div><?php
						}
					}
				}
				...
				fsdriving_show_layout($post_info);
				?></div><?php
			}
		}
	}
...
if ( !function_exists('fsdriving_get_no_image') ) {
	function fsdriving_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = fsdriving_get_theme_option( 'no_image' );
			// Remove false in next line if you want display theme-specific image in the posts without featured image
			if (false && empty($img)) $img = fsdriving_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('fsdriving_add_bg_in_post_nav') ) {
	function fsdriving_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = fsdriving_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), fsdriving_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), fsdriving_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'fsdriving-main', $css );
	}
}

// Show related posts
if ( !function_exists('fsdriving_show_related_posts') ) {
	function fsdriving_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('Latest News', 'fsdriving');
				?></h3>
...
if ( !function_exists('fsdriving_show_portfolio_posts') ) {
	function fsdriving_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? fsdriving_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = fsdriving_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = fsdriving_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the fsdriving_ajax_get_posts action
if ( !function_exists( 'fsdriving_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_fsdriving_ajax_get_posts',			'fsdriving_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_fsdriving_ajax_get_posts',	'fsdriving_ajax_get_posts_callback');
	function fsdriving_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			fsdriving_storage_set('blog_mode', 'blog');
			fsdriving_storage_set('options_meta', get_post_meta($id, 'fsdriving_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => fsdriving_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'fsdriving');
		}
...
// Show pagination
if ( !function_exists('fsdriving_show_pagination') ) {
	function fsdriving_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = fsdriving_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'fsdriving' ),
				'next_text' => esc_html__( '>', 'fsdriving' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'fsdriving' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'fsdriving'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'fsdriving') : esc_html__('Newest posts', 'fsdriving') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'fsdriving') : esc_html__('Older posts', 'fsdriving'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'fsdriving_single_comments_field' ) ) {
    function fsdriving_single_comments_field( $args ) {
