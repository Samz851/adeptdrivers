...
if (!function_exists('fsdriving_trx_addons_theme_setup1')) {
	add_action( 'after_setup_theme', 'fsdriving_trx_addons_theme_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'fsdriving_trx_addons_theme_setup1', 8 );
	function fsdriving_trx_addons_theme_setup1() {
		if (fsdriving_exists_trx_addons()) {
			add_filter( 'fsdriving_filter_list_posts_types',	'fsdriving_trx_addons_list_post_types');
			add_filter( 'fsdriving_filter_list_header_styles','fsdriving_trx_addons_list_header_styles');
			add_filter( 'fsdriving_filter_list_footer_styles','fsdriving_trx_addons_list_footer_styles');
		}
	}
}
...
if (!function_exists('fsdriving_trx_addons_theme_setup9')) {
	add_action( 'after_setup_theme', 'fsdriving_trx_addons_theme_setup9', 9 );
	function fsdriving_trx_addons_theme_setup9() {
		if (fsdriving_exists_trx_addons()) {
			add_filter( 'trx_addons_sc_atts',							'fsdriving_trx_addons_sc_atts', 10, 2);
			add_filter( 'trx_addons_sc_output',							'fsdriving_trx_addons_sc_output', 10, 4);
			add_filter( 'trx_addons_filter_featured_image',				'fsdriving_trx_addons_featured_image', 10, 2);
			add_filter( 'trx_addons_filter_no_image',					'fsdriving_trx_addons_no_image' );
			add_filter( 'trx_addons_filter_get_list_icons',				'fsdriving_trx_addons_get_list_icons', 10, 2 );
			add_filter( 'trx_addons_filter_slider_title',				'fsdriving_trx_addons_slider_title', 10, 2 );
			add_filter( 'trx_addons_filter_meta_box_fields',			'fsdriving_trx_addons_options_override_fields', 10, 2);
			add_action( 'wp_enqueue_scripts', 							'fsdriving_trx_addons_frontend_scripts', 1100 );
			add_filter( 'fsdriving_filter_query_sort_order',	 			'fsdriving_trx_addons_query_sort_order', 10, 3);
			add_filter( 'fsdriving_filter_merge_scripts',					'fsdriving_trx_addons_merge_scripts');
			add_filter( 'fsdriving_filter_get_css',						'fsdriving_trx_addons_get_css', 10, 4);
			add_filter( 'fsdriving_filter_prepare_css',					'fsdriving_trx_addons_prepare_css', 10, 2);
			add_filter( 'fsdriving_filter_prepare_js',					'fsdriving_trx_addons_prepare_js', 10, 2);
			add_filter( 'fsdriving_filter_get_post_categories',		 	'fsdriving_trx_addons_get_post_categories');
			add_filter( 'fsdriving_filter_get_post_date',		 			   'fsdriving_trx_addons_get_post_date');
			add_filter( 'trx_addons_filter_get_post_date',		 		'fsdriving_trx_addons_get_post_date_wrap');
			add_filter( 'fsdriving_filter_post_type_taxonomy',			'fsdriving_trx_addons_post_type_taxonomy', 10, 2 );
			if (is_admin()) {
				add_filter( 'fsdriving_filter_allow_override', 			'fsdriving_trx_addons_allow_override', 10, 2);
				add_filter( 'fsdriving_filter_allow_theme_icons', 		'fsdriving_trx_addons_allow_theme_icons', 10, 2);
				add_filter( 'trx_addons_sc_map',						'fsdriving_trx_addons_sc_map', 10, 2);
			} else {
				add_filter( 'trx_addons_filter_theme_logo',				'fsdriving_trx_addons_theme_logo');
				add_filter( 'trx_addons_filter_post_meta',				'fsdriving_trx_addons_post_meta', 10, 2);
				add_filter( 'fsdriving_filter_get_mobile_menu',			'fsdriving_trx_addons_get_mobile_menu');
				add_filter( 'fsdriving_filter_detect_blog_mode',			'fsdriving_trx_addons_detect_blog_mode' );
				add_filter( 'fsdriving_filter_get_blog_title', 			'fsdriving_trx_addons_get_blog_title');
				add_action( 'fsdriving_action_login',						'fsdriving_trx_addons_action_login', 10, 2);
				add_action( 'fsdriving_action_search',					'fsdriving_trx_addons_action_search', 10, 3);
				add_action( 'fsdriving_action_breadcrumbs',				'fsdriving_trx_addons_action_breadcrumbs');
				add_action( 'fsdriving_action_show_layout',				'fsdriving_trx_addons_action_show_layout', 10, 1);
			}
		}
		
		// Add this filter any time: if plugin exists - load plugin's styles, if not exists - load layouts.css instead plugin's styles
		add_filter( 'fsdriving_filter_merge_styles',						'fsdriving_trx_addons_merge_styles');
		
		if (is_admin()) {
			add_filter( 'fsdriving_filter_tgmpa_required_plugins',		'fsdriving_trx_addons_tgmpa_required_plugins' );
			add_action( 'admin_enqueue_scripts', 						'fsdriving_trx_addons_editor_load_scripts_admin');
		}
	}
}

// Filter to add in the required plugins list
if ( !function_exists( 'fsdriving_trx_addons_tgmpa_required_plugins' ) ) {
	//Handler of the add_filter('fsdriving_filter_tgmpa_required_plugins',	'fsdriving_trx_addons_tgmpa_required_plugins');
	function fsdriving_trx_addons_tgmpa_required_plugins($list=array()) {
		if (in_array('trx_addons', fsdriving_storage_get('required_plugins'))) {
			$path = fsdriving_get_file_dir('plugins/trx_addons/trx_addons.zip');
			$list[] = array(
					'name' 		=> esc_html__('ThemeREX Addons', 'fsdriving'),
					'slug' 		=> 'trx_addons',
					'version'	=> '111.6.18.1',
...
if ( !function_exists( 'fsdriving_exists_trx_addons' ) ) {
	function fsdriving_exists_trx_addons() {
		return defined('TRX_ADDONS_VERSION');
	}
}

// Return true if team is supported
if ( !function_exists( 'fsdriving_exists_team' ) ) {
	function fsdriving_exists_team() {
		return defined('TRX_ADDONS_CPT_TEAM_PT');
	}
}

// Return true if services is supported
if ( !function_exists( 'fsdriving_exists_services' ) ) {
	function fsdriving_exists_services() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT');
	}
}

// Return true if portfolio is supported
if ( !function_exists( 'fsdriving_exists_portfolio' ) ) {
	function fsdriving_exists_portfolio() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT');
	}
}

// Return true if courses is supported
if ( !function_exists( 'fsdriving_exists_courses' ) ) {
	function fsdriving_exists_courses() {
		return defined('TRX_ADDONS_CPT_COURSES_PT');
	}
}

// Return true if layouts is supported
if ( !function_exists( 'fsdriving_exists_layouts' ) ) {
	function fsdriving_exists_layouts() {
		return defined('TRX_ADDONS_CPT_LAYOUTS_PT');
	}
}

// Return true if dishes is supported
if ( !function_exists( 'fsdriving_exists_dishes' ) ) {
	function fsdriving_exists_dishes() {
		return defined('TRX_ADDONS_CPT_DISHES_PT');
	}
}

// Return true if sport is supported
if ( !function_exists( 'fsdriving_exists_sport' ) ) {
	function fsdriving_exists_sport() {
...
if ( !function_exists( 'fsdriving_is_team_page' ) ) {
	function fsdriving_is_team_page() {
		return defined('TRX_ADDONS_CPT_TEAM_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_is_services_page' ) ) {
	function fsdriving_is_services_page() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_is_portfolio_page' ) ) {
	function fsdriving_is_portfolio_page() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_is_courses_page' ) ) {
	function fsdriving_is_courses_page() {
		return defined('TRX_ADDONS_CPT_COURSES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_is_dishes_page' ) ) {
	function fsdriving_is_dishes_page() {
		return defined('TRX_ADDONS_CPT_DISHES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_is_sport_page' ) ) {
	function fsdriving_is_sport_page() {
		return defined('TRX_ADDONS_CPT_COMPETITIONS_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'fsdriving_trx_addons_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'fsdriving_trx_addons_frontend_scripts', 1100 );
	function fsdriving_trx_addons_frontend_scripts() {
		if (fsdriving_exists_trx_addons()) {
			if (fsdriving_is_on(fsdriving_get_theme_option('debug_mode')) && fsdriving_get_file_dir('plugins/trx_addons/trx_addons.css')!='') {
				wp_enqueue_style( 'fsdriving-trx_addons',  fsdriving_get_file_url('plugins/trx_addons/trx_addons.css'), array(), null );
				wp_enqueue_style( 'fsdriving-trx_addons_editor',  fsdriving_get_file_url('plugins/trx_addons/trx_addons.editor.css'), array(), null );
			}
			if (fsdriving_is_on(fsdriving_get_theme_option('debug_mode')) && fsdriving_get_file_dir('plugins/trx_addons/trx_addons.js')!='')
				wp_enqueue_script( 'fsdriving-trx_addons', fsdriving_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		} else {
			// Load custom layouts from the theme if plugin not exists
			if ( fsdriving_is_on(fsdriving_get_theme_option('debug_mode')) ) {
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/layouts.css') );
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/logo.css') );
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/menu.css') );
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/search.css') );
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/title.css') );
				wp_enqueue_style( 'fsdriving-layouts', fsdriving_get_file_url('plugins/trx_addons/layouts/featured.css') );
			}
		}
	}
}
...
// Merge custom styles
if ( !function_exists( 'fsdriving_trx_addons_merge_styles' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_merge_styles', 'fsdriving_trx_addons_merge_styles');
	function fsdriving_trx_addons_merge_styles($list) {
...
if ( !function_exists( 'fsdriving_trx_addons_merge_scripts' ) ) {
	//Handler of the add_filter('fsdriving_filter_merge_scripts', 'fsdriving_trx_addons_merge_scripts');
	function fsdriving_trx_addons_merge_scripts($list) {
...
if ( !function_exists( 'fsdriving_trx_addons_detect_blog_mode' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_detect_blog_mode', 'fsdriving_trx_addons_detect_blog_mode' );
	function fsdriving_trx_addons_detect_blog_mode($mode='') {
		if ( fsdriving_is_team_page() )
			$mode = 'team';
		else if ( fsdriving_is_services_page() )
			$mode = 'services';
		else if ( fsdriving_is_portfolio_page() )
			$mode = 'portfolio';
		else if ( fsdriving_is_courses_page() )
			$mode = 'courses';
		else if ( fsdriving_is_dishes_page() )
			$mode = 'dishes';
		else if ( fsdriving_is_sport_page() )
			$mode = 'sport';
		return $mode;
	}
}
...
if ( !function_exists( 'fsdriving_trx_addons_list_post_types' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_list_posts_types', 'fsdriving_trx_addons_list_post_types');
	function fsdriving_trx_addons_list_post_types($list=array()) {
...
if ( !function_exists( 'fsdriving_trx_addons_post_type_taxonomy' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_post_type_taxonomy',	'fsdriving_trx_addons_post_type_taxonomy', 10, 2 );
	function fsdriving_trx_addons_post_type_taxonomy($tax='', $post_type='') {
...
if ( !function_exists( 'fsdriving_trx_addons_get_post_categories' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_get_post_categories', 		'fsdriving_trx_addons_get_post_categories');
	function fsdriving_trx_addons_get_post_categories($cats='') {

		if ( defined('TRX_ADDONS_CPT_TEAM_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_TEAM_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_TEAM_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_SERVICES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_SERVICES_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_PORTFOLIO_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_PORTFOLIO_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_PORTFOLIO_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_COURSES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_DISHES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_DISHES_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_DISHES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_COMPETITIONS_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COMPETITIONS_PT) {
				$cats = fsdriving_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_COMPETITIONS_TAXONOMY);
			}
		}
		return $cats;
	}
...
if ( !function_exists( 'fsdriving_trx_addons_list_header_styles' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_list_header_styles', 'fsdriving_trx_addons_list_header_styles');
	function fsdriving_trx_addons_list_header_styles($list=array()) {
		if (fsdriving_exists_layouts()) {
			$layouts = fsdriving_get_list_posts(false, array(
...
if ( !function_exists( 'fsdriving_trx_addons_list_footer_styles' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_list_footer_styles', 'fsdriving_trx_addons_list_footer_styles');
	function fsdriving_trx_addons_list_footer_styles($list=array()) {
		if (fsdriving_exists_layouts()) {
			$layouts = fsdriving_get_list_posts(false, array(
							'post_type' => TRX_ADDONS_CPT_LAYOUTS_PT,
							'meta_key' => 'trx_addons_layout_type',
...
if ( !function_exists( 'fsdriving_trx_addons_get_post_date_wrap' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_post_date', 'fsdriving_trx_addons_get_post_date_wrap');
	function fsdriving_trx_addons_get_post_date_wrap($dt='') {
		return apply_filters('fsdriving_filter_get_post_date', $dt);
	}
}

// Show date of the courses
if ( !function_exists( 'fsdriving_trx_addons_get_post_date' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_get_post_date', 'fsdriving_trx_addons_get_post_date');
	function fsdriving_trx_addons_get_post_date($dt='') {

		if ( defined('TRX_ADDONS_CPT_COURSES_PT') && get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
			$meta = get_post_meta(get_the_ID(), 'trx_addons_options', true);
			$dt = $meta['date'];
			$dt = sprintf($dt < date('Y-m-d') 
					? esc_html__('Started on %s', 'fsdriving') 
					: esc_html__('Starting %s', 'fsdriving'), 
			...
			if (false) {
				$meta = get_post_meta(get_the_ID(), 'trx_addons_options', true);
				$dt = !empty($meta['birthday']) ? sprintf(esc_html__('Birthday: %s', 'fsdriving'), date(get_option('date_format'), strtotime($meta['birthday']))) : '';
			} else
				$dt = '';
		}
		return $dt;
...
if (!function_exists('fsdriving_trx_addons_allow_override')) {
	//Handler of the add_filter( 'fsdriving_filter_allow_override', 'fsdriving_trx_addons_allow_override', 10, 2);
	function fsdriving_trx_addons_allow_override($allow, $post_type) {
		return $allow
...
if (!function_exists('fsdriving_trx_addons_allow_theme_icons')) {
	//Handler of the add_filter( 'fsdriving_filter_allow_theme_icons', 'fsdriving_trx_addons_allow_theme_icons', 10, 2);
	function fsdriving_trx_addons_allow_theme_icons($allow, $post_type) {
		return $allow
					|| (defined('TRX_ADDONS_CPT_LAYOUTS_PT') && $post_type==TRX_ADDONS_CPT_LAYOUTS_PT);
	}
}

// Add fields into options override
if (!function_exists('fsdriving_trx_addons_options_override_fields')) {
	//Handler of the add_filter( 'trx_addons_filter_options_override_fields', 'fsdriving_trx_addons_options_override_fields', 10, 2);
	function fsdriving_trx_addons_options_override_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'fsdriving'),
				"desc" => wp_kses_data( __("Team member's email", 'fsdriving') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
if ( !function_exists( 'fsdriving_trx_addons_editor_load_scripts_admin' ) ) {
	//Handler of the add_action("admin_enqueue_scripts", 'fsdriving_trx_addons_editor_load_scripts_admin');
	function fsdriving_trx_addons_editor_load_scripts_admin() {
		// Add styles in the WP text editor
		add_editor_style( array(
							fsdriving_get_file_url('plugins/trx_addons/trx_addons.editor.css')
							)
						 );	
	}
}
...
if ( !function_exists( 'fsdriving_trx_addons_sc_atts' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_atts', 'fsdriving_trx_addons_sc_atts', 10, 2);
	function fsdriving_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'fsdriving_trx_addons_sc_map' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_map', 'fsdriving_trx_addons_sc_map', 10, 2);
...
if ( !function_exists( 'fsdriving_trx_addons_sc_output' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_output', 'fsdriving_trx_addons_sc_output', 10, 4);
	function fsdriving_trx_addons_sc_output($output, $sc, $atts, $content) {
...
if ( !function_exists( 'fsdriving_trx_addons_slider_title' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_slider_title',	'fsdriving_trx_addons_slider_title', 10, 2 );
	function fsdriving_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if (!function_exists('fsdriving_sc_layouts_showed')) {
	function fsdriving_sc_layouts_showed($name, $val=null) {
		if (function_exists('trx_addons_sc_layouts_showed')) {
			...
			if ($val!==null)
				return fsdriving_storage_set_array('sc_layouts_components', $name, $val);
			else
				return fsdriving_storage_get_array('sc_layouts_components', $name);
		}
	}
}

// Return image size multiplier
if (!function_exists('fsdriving_get_retina_multiplier')) {
	function fsdriving_get_retina_multiplier($force_retina=0) {
		static $mult = 0;
...
if (!function_exists('fsdriving_build_slider_layout')) {
	function fsdriving_build_slider_layout($args) {
...
if ( !function_exists( 'fsdriving_trx_addons_featured_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_featured_image', 'fsdriving_trx_addons_featured_image', 10, 2);
	function fsdriving_trx_addons_featured_image($processed=false, $args=array()) {
		$args['show_no_image'] = true;
		$args['singular'] = false;
		$args['hover'] = isset($args['hover']) && $args['hover']=='' ? '' : fsdriving_get_theme_option('image_hover');
		fsdriving_show_post_featured($args);
		return true;
	}
}

// Return theme specific 'no-image' picture
if ( !function_exists( 'fsdriving_trx_addons_no_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_no_image', 'fsdriving_trx_addons_no_image');
	function fsdriving_trx_addons_no_image($no_image='') {
		return fsdriving_get_no_image($no_image);
	}
}

// Return theme-specific icons
if ( !function_exists( 'fsdriving_trx_addons_get_list_icons' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_list_icons', 'fsdriving_trx_addons_get_list_icons', 10, 2 );
	function fsdriving_trx_addons_get_list_icons($list, $prepend_inherit) {
		return fsdriving_get_list_icons($prepend_inherit);
	}
}

// Return links to the social profiles
if (!function_exists('fsdriving_get_socials_links')) {
	function fsdriving_get_socials_links($style='icons') {
...
if (!function_exists('fsdriving_get_share_links')) {
	function fsdriving_get_share_links($args=array()) {
...
if (!function_exists('fsdriving_show_share_links')) {
	function fsdriving_show_share_links($args=array()) {
...
if (!function_exists('fsdriving_get_category_image')) {
	function fsdriving_get_category_image($term_id=0) {
...
if (!function_exists('fsdriving_get_category_icon')) {
	function fsdriving_get_category_icon($term_id=0) {
...
if (!function_exists('fsdriving_get_post_counters')) {
	function fsdriving_get_post_counters($counters='views') {
...
if (!function_exists('fsdriving_get_list_animations_in')) {
	function fsdriving_get_list_animations_in() {
...
if (!function_exists('fsdriving_get_animation_classes')) {
	function fsdriving_get_animation_classes($animation, $speed='normal', $loop='none') {
...
if (!function_exists('fsdriving_get_comment_counters')) {
	function fsdriving_get_comment_counters($counters = 'likes') {
...
if (!function_exists('fsdriving_show_comment_counters')) {
	function fsdriving_show_comment_counters($counters = 'likes') {
...
if (!function_exists('fsdriving_trx_addons_query_sort_order')) {
	//Handler of the add_filter('fsdriving_filter_query_sort_order', 'fsdriving_trx_addons_query_sort_order', 10, 3);
	function fsdriving_trx_addons_query_sort_order($q=array(), $orderby='date', $order='desc') {
		if ($orderby == 'views') {
...
if ( !function_exists( 'fsdriving_trx_addons_theme_logo' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_theme_logo', 'fsdriving_trx_addons_theme_logo');
	function fsdriving_trx_addons_theme_logo($logo) {
		return fsdriving_get_logo_image();
	}
}

// Return theme-specific post meta to the plugin
if ( !function_exists( 'fsdriving_trx_addons_post_meta' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_post_meta',	'fsdriving_trx_addons_post_meta', 10, 2);
	function fsdriving_trx_addons_post_meta($meta, $args=array()) {
		return fsdriving_show_post_meta($args);
	}
}
	
// Redirect action 'get_mobile_menu' to the plugin
// Return stored items as mobile menu
if ( !function_exists( 'fsdriving_trx_addons_get_mobile_menu' ) ) {
	//Handler of the add_filter("fsdriving_filter_get_mobile_menu", 'fsdriving_trx_addons_get_mobile_menu');
	function fsdriving_trx_addons_get_mobile_menu($menu) {
		return apply_filters('trx_addons_filter_get_mobile_menu', $menu);
	}
}

// Redirect action 'login' to the plugin
if (!function_exists('fsdriving_trx_addons_action_login')) {
	//Handler of the add_action( 'fsdriving_action_login',		'fsdriving_trx_addons_action_login', 10, 2);
	function fsdriving_trx_addons_action_login($link_text='', $link_title='') {
		do_action( 'trx_addons_action_login', $link_text, $link_title );
	}
}

// Redirect action 'search' to the plugin
if (!function_exists('fsdriving_trx_addons_action_search')) {
	//Handler of the add_action( 'fsdriving_action_search', 'fsdriving_trx_addons_action_search', 10, 3);
	function fsdriving_trx_addons_action_search($style, $class, $ajax) {
		do_action( 'trx_addons_action_search', $style, $class, $ajax );
	}
}

// Redirect action 'breadcrumbs' to the plugin
if (!function_exists('fsdriving_trx_addons_action_breadcrumbs')) {
	//Handler of the add_action( 'fsdriving_action_breadcrumbs',	'fsdriving_trx_addons_action_breadcrumbs');
	function fsdriving_trx_addons_action_breadcrumbs() {
		do_action( 'trx_addons_action_breadcrumbs' );
	}
}

// Redirect action 'show_layout' to the plugin
if (!function_exists('fsdriving_trx_addons_action_show_layout')) {
	//Handler of the add_action( 'fsdriving_action_show_layout', 'fsdriving_trx_addons_action_show_layout', 10, 1);
	function fsdriving_trx_addons_action_show_layout($layout_id='') {
		do_action( 'trx_addons_action_show_layout', $layout_id );
	}
}

// Redirect filter 'get_blog_title' to the plugin
if ( !function_exists( 'fsdriving_trx_addons_get_blog_title' ) ) {
	//Handler of the add_filter( 'fsdriving_filter_get_blog_title', 'fsdriving_trx_addons_get_blog_title');
	function fsdriving_trx_addons_get_blog_title($title='') {
		return apply_filters('trx_addons_filter_get_blog_title', $title);
	}
}

// Redirect filter 'prepare_css' to the plugin
if (!function_exists('fsdriving_trx_addons_prepare_css')) {
	//Handler of the add_filter( 'fsdriving_filter_prepare_css',	'fsdriving_trx_addons_prepare_css', 10, 2);
	function fsdriving_trx_addons_prepare_css($css='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_css', $css, $remove_spaces );
	}
}

// Redirect filter 'prepare_js' to the plugin
if (!function_exists('fsdriving_trx_addons_prepare_js')) {
	//Handler of the add_filter( 'fsdriving_filter_prepare_js',	'fsdriving_trx_addons_prepare_js', 10, 2);
	function fsdriving_trx_addons_prepare_js($js='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_js', $js, $remove_spaces );
	}
}

// Return text for the "I agree ..." checkbox
if ( ! function_exists( 'fsdriving_trx_addons_privacy_text' ) ) {
    add_filter( 'trx_addons_filter_privacy_text', 'fsdriving_trx_addons_privacy_text' );
    function fsdriving_trx_addons_privacy_text( $text='' ) {
        return fsdriving_get_privacy_text();
    }
}

// Add theme-specific options to the post's options
if (!function_exists('fsdriving_trx_addons_override_options')) {
    add_filter( 'trx_addons_filter_override_options', 'fsdriving_trx_addons_override_options');
    function fsdriving_trx_addons_override_options($options=array()) {
        return apply_filters('fsdriving_filter_override_options', $options);
    }
}
...
if (!function_exists('fsdriving_trx_addons_get_css')) {
	//Handler of the add_filter('fsdriving_filter_get_css', 'fsdriving_trx_addons_get_css', 10, 4);
	function fsdriving_trx_addons_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_item_button a,
.sc_form button,
.sc_courses_default .trx_addons_hover_links a,
.sc_button,
.sc_item_subtitle,
.sc_services_item_title_number,
.sc_price_link,
.sc_price_subtitle,
.sc_action_item_link{
...
.sc_blogger_classic .sc_blogger_slider .slider_controls_wrap a,
.sc_courses_default .sc_courses_slider .slider_controls_wrap a,
.sc_slider_controls .slider_prev,
.sc_slider_controls .slider_next {
	background-color: {$colors['bg_color']};
	color: {$colors['text_dark']};
}
.sc_blogger_classic .sc_blogger_slider .slider_controls_wrap a:hover,
.sc_courses_default .sc_courses_slider .slider_controls_wrap a:hover,
.sc_slider_controls .slider_prev:hover,
.sc_slider_controls .slider_next:hover {
	background-color: {$colors['bg_color']};
	color: {$colors['text_hover']};
} 
...
.sc_item_subtitle.sc_item_title_style_blue {
	color:{$colors['alter_link']};
}

.sc_item_subtitle.sc_item_title_style_red {
	color:{$colors['alter_hover']};
}
...
.sc_item_button a.sc_button_alter:not(.sc_button_bg_image) {
	color:{$colors['alter_light']};
	background-color: {$colors['accent4']};
}
.sc_item_button a.sc_button_alter:not(.sc_button_bg_image):hover {
	color:{$colors['alter_light']};
	background-color:{$colors['accent2']};
}
.sc_item_button a:not(.sc_button_bg_image) .sc_button_icon > i {
	color:{$colors['alter_light']};
}
...
.sc_blogger_classic .sc_blogger_item .post_icon {
	background-color: {$colors['accent2']};
}
.sc_blogger_classic .sc_blogger_item .post_icon i {
	color: {$colors['alter_light']};
}
.post_featured.sc_blogger_item_featured.hover_dots .icons:after {
	color: {$colors['accent2']};
}
...
.sc_countdown_item.sc_countdown_days .sc_countdown_digits span {
	color: {$colors['accent2']};
}
/* Courses */
...
.sc_courses_default .sc_courses_item_thumb:hover .sc_courses_hover_links a:hover {
...
.sc_icons_size_small.sc_align_left .sc_icons_item .sc_icons_icon {
	background-color: {$colors['accent2']};
	color: {$colors['alter_light']};
}
.sc_icons_size_small.sc_align_left .sc_icons_item .sc_icons_icon:hover:before,
.sc_icons_size_small.sc_align_left .sc_icons_item .sc_icons_icon:before {
	color: {$colors['alter_light']};
}
...
.sc_price_info ul[class*="trx_addons_list"] > li:before {
	background-color: {$colors['accent3']};
}
.sc_price_info .sc_price_details {
	border-bottom-color: {$colors['accent3']};
}
...
.scheme_self.sc_price.with_subtitle,
.scheme_self.sc_price:hover {
	background-color: {$colors['alter_bg_color']};
}
.scheme_self.sc_price:hover .sc_price_info .sc_price_price {
...
.sc_services_default .sc_services_item_image_icon:hover {
	background-color: {$colors['alter_bg_hover']};
}
...
.sc_testimonials_item_content:after {
	border-top-color: {$colors['alter_bd_hover']};
}
...
.sc_team_short .trx_addons_hover_content .trx_addons_hover_icon_link:hover {
	background-color: transparent;
	color: {$colors['accent2']} !important;
}
.scheme_self.sc_team_short .sc_team_item_header {
	background-color: {$colors['bg_color']};
}
.scheme_self.sc_team_short .sc_team_item_title a,
.scheme_self.sc_team_short .sc_team_item_title {
	color: {$colors['text_dark']};
}
.sc_team_short .sc_team_item_title a:hover {
	color: {$colors['text_hover']};
}
.scheme_self.sc_team_short .sc_team_item_subtitle {
	color: {$colors['text_light']};
}
...
.top_panel_custom_577 .fsds_header.fsds_menu {
	background-color: {$colors['accent3']};
}
